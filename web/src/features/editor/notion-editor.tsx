...
/**
 * The product's one Notion-style editor, on TipTap.
 *
 * Live rich-text formatting, `/` commands, `@` mentions, task lists, tables,
 * image upload (paste / drop / picker), ⌘K links and link-preview cards. Emits
 * HTML; mentions serialize with a literal `@Name` so mention extraction on the
 * stored string keeps working.
 *
 * One component rather than one per surface: a task description, a doc page and
 * a campaign brief are the same editor at different sizes. `variant` is that
 * size — "compact" for a field inside a form, "page" for a document someone
 * writes into for an hour.
 */
export function NotionEditor({
  ...
  onCommit: () => void;
  /** Compact only: the editor's resting and maximum height, in text rows. */
  minRows?: number;
  maxRows?: number;
  ...
  placeholder?: string;
  /**
   * "compact" caps the height and scrolls inside itself — right for a field.
   * "page" grows with its content and sets document type sizes, so a long doc
   * scrolls with the page instead of trapping a scrollbar inside a box.
   */
  variant?: "compact" | "page";
  /** Cards under the editor for any URLs in the text. Noise on a long doc. */
  ...
  useEffect(() => {
    ...
    // Only on the editor's arrival: re-running on every autoFocus flip would
    // steal the caret back from wherever the user has since clicked.
  ...
  return (
    <div className="rd" ref={rdRef}>
      ...
      {linkPreviews ? <LinkPreviews text={plainText} /> : null}
    </div>
  );
}
