...
import type { ChatAttachment, ChatMemberInfo } from "@/features/chat/use-chat";
import {
  useChatReactions,
  useToggleReaction,
  useEditMessage,
...
function ChatThreadPageInner() {
  ...
  /**
   * A PUBLIC channel has no real roster: every team member can read and post
   * in it, and a `chat_channel_members` row only appears once someone opens it
   * (the mark-read upsert). So "everyone here" means the whole workspace —
   * fanning out to those rows would ping only the handful who happened to
   * visit. Private channels, group messages and DMs use their actual roster.
   */
  const isOpenChannel = channel?.kind === "channel" && !channel.is_private;
  const teamUserIds = useMemo(
    () =>
      (teamMembers ?? [])
        .filter((m) => m.active && m.user && m.member_type !== "guest")
        .map((m) => m.user!.id),
    [teamMembers],
  );
  /** Who "@all" reaches in this conversation. */
  const broadcastUserIds = useMemo(
    () => (isOpenChannel ? teamUserIds : members.map((m) => m.user_id)),
    [isOpenChannel, teamUserIds, members],
  );

  /**
   * What the members popover lists. Shaped like a `chat_channel_members` row so
   * one renderer serves both cases: a public channel synthesises rows for the
   * workspace (there is no stored roster), everything else uses the real one.
   */
  const roster = useMemo<ChatMemberInfo[]>(() => {
    if (!isOpenChannel) return members;
    return (teamMembers ?? [])
      .filter((m) => m.active && m.user && m.member_type !== "guest")
      .map((m) => ({
        id: `team:${m.id}`,
  ...
  }, [isOpenChannel, members, teamMembers]);
  ...
  const submit = () => {
    ...
    send.mutate({ body: text, attachments: sentAttachments }, {
      ...
      onSuccess: () => {
        ...
        // "@all" broadcasts to everyone this conversation addresses — the whole
        // workspace in a public channel, the roster anywhere else — minus the
        // sender and anyone already mentioned by name, so nobody is pinged twice.
        if (ALL_MENTION_RE.test(text)) {
          const explicit = new Set(
            extractMentions(text, mentionMembers).userIds,
          );
          void notifyUsers({
            userIds: broadcastUserIds.filter((id) => !explicit.has(id)),
          ...
          <Popover
            ...
            content={
              <div style={{ width: 280, maxHeight: 380, overflowY: "auto" }}>
                ...
                >
                  {isOpenChannel
                    ? `Everyone in this workspace · ${roster.length}`
                ...
                {isOpenChannel ? (
                  ...
                  >
                    Anyone in the workspace can read and post here, so @all
                    reaches all of them.
                  </div>
                ) : null}
                {roster.map((m) => {
                  const isCreator = m.user_id === channel?.created_by;
                  // A public channel has no roster to manage — membership is
                  // just "is in the workspace", so nobody is removable here.
                  const canRemove =
                    isOpenChannel || isCreator
                      ? false
